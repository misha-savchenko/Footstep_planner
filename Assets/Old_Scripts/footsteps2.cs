...
public class footsteps2 : MonoBehaviour {
    ...
    {
        float cost = 1/Vector3.Distance(node_candidate,new Vector2(temp_goal_location[0], temp_goal_location[2]));
        return cost;
    }
    ...
    void Footstep(GameObject foot, GameObject other_foot, string foot_name) {
        ...
        Vector2 toward = (new Vector2(temp_goal_location[0], temp_goal_location[2]) - new Vector2(foot.transform.position[0], foot.transform.position[2]));


        float angle = Vector2.Angle( foot.transform.localEulerAngles, toward);
